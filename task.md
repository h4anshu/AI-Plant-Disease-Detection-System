...
- [ ] **Earth Engine setup for field health** (`docs/GEE_SETUP.md`, about 30 min): register `plant-disease-503711` for noncommercial Earth Engine (licence check first), run the step-2 test, create the `geo-service` service account with its 2 roles, set a daily EECU cap, optionally request ALU access. Then send the non-secret answers from section 6 plus **3 real field coordinates** (owners' consent). The field-health code starts after that.
